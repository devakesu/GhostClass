...
class _AddAttendanceDialogState extends ConsumerState<AddAttendanceDialog> {
  ...
  String _getSessionName(Map<String, dynamic> meta, String key, AttendanceSession obj, int idx) {
    ...
    if (official != null) {
      ...
      official.forEach((key, obj) {
        if (obj.course != null && obj.course != 0) {
          occupied.add(utils.normalizeSession(_getSessionName(data.attendance.sessions, key, obj, idx)));
        }
        idx++;
      });
    }
    ...
    if (official != null) {
      ...
      for (final e in official.entries) {
        if (e.value.course != null && e.value.course != 0) {
          if (utils.normalizeSession(_getSessionName(data.attendance.sessions, e.key, e.value, idx)) == target) { 
            blocked = true; 
            break; 
          }
        }
  ...
  Color _getUniformFieldColor() {
    return Theme.of(context).inputDecorationTheme.fillColor ?? Theme.of(context).colorScheme.surface;
  }
  ...
  Widget _buildSubjectSelectorButton(DashboardData? data, Color primary) {
    ...
    final isDisabled = selectedCourse != null && _isCourseDisabled(selectedCourse, data);
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            Icon(LucideIcons.chevronDown, size: 16, color: Theme.of(context).colorScheme.onSurface.withValues(alpha: 0.4)),
          ],
        ),
      ),
    );
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 12),
              Divider(color: Theme.of(context).colorScheme.onSurface.withValues(alpha: 0.05)),
              Flexible(
                child: Scrollbar(
                              ...
                              children: [
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      Text(
                                        isDisabled ? '${c.name} (Disabled)' : c.name,
                                        style: GoogleFonts.manrope(
                                          fontSize: 14,
                                          fontWeight: isSelected ? FontWeight.w800 : FontWeight.w600,
                                          fontStyle: isDisabled ? FontStyle.italic : FontStyle.normal,
                                          color: isDisabled 
                                            ? Theme.of(context).colorScheme.onSurface.withValues(alpha: 0.4)
                                            : (isSelected ? primary : Theme.of(context).colorScheme.onSurface),
                                        ),
                                      ),
                                ...
                                if (isSelected)
                                  Icon(LucideIcons.checkCircle2, color: primary, size: 18),
                              ],
                            ),
                          ),
                        ),
  ...
  Widget _buildRemarksField(Color primary) {
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        prefixIcon: Icon(LucideIcons.textCursor, size: 18, color: Theme.of(context).colorScheme.onSurface.withValues(alpha: 0.4)),
        border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: Theme.of(context).colorScheme.onSurface.withValues(alpha: 0.12))),
        enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: Theme.of(context).colorScheme.onSurface.withValues(alpha: 0.12))),
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        ...
        child: _isSubmitting 
          ...
          : const Text('Add Record', style: TextStyle(fontWeight: FontWeight.bold)),
      ),
    );
  }
