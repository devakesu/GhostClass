...
void main() {
  group('NetworkUtils', () {
    ...
    test('validates mkcert development certificate', () {
      when(() => mockCert.subject).thenReturn('CN=mkcert development certificate,O=mkcert');
      final isValid = NetworkUtils.validateCertificateHostname(mockCert, 'localhost', 8080);
      expect(isValid, true);
    });
    ...
    test('evaluates wildcard certificate prefix and suffix matching safely', () {
      // Hits startsWith('*.') and substring extraction logic
      when(() => mockCert.subject).thenReturn('cn=*.localhost,O=TestWildcard');
      final isValid = NetworkUtils.validateCertificateHostname(mockCert, 'localhost', 8080);
      ...
      final isOtherValid = NetworkUtils.validateCertificateHostname(mockCert, 'localhost', 8080);
