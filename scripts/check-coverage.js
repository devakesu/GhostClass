...
for (const file in coverage) {
  ...
  const bCovered = Object.values(b).reduce((acc, curr) => acc + curr.filter(v => v > 0).length, 0);
...
allFiles.slice(0, 10).forEach(f => {
    console.log(`${f.file.replace(process.cwd(), '')}: S:${f.sPct.toFixed(2)}% F:${f.fPct.toFixed(2)}% B:${f.bPct.toFixed(2)}%`);
});
