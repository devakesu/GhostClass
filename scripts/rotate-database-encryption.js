...
function printHeader() {
  ...
  console.log(
    `${colors.bold}       GhostClass Standalone Database Encryption Key Rotator       ${colors.reset}`,
  );
  console.log(
    `${colors.bold}${colors.cyan}=====================================================================${colors.reset}\n`,
  );
...
function printUsageAndExit(errorMessage) {
  ...
  console.log(
    `  ${colors.bold}SUPABASE_URL${colors.reset}         → Real production Supabase URL (https://*.supabase.co)`,
  );
  console.log(
    `  ${colors.bold}SUPABASE_SECRET_KEY${colors.reset}  → Supabase Service Role Secret Key (sb_secret_*)`,
  );
  console.log(
    `  ${colors.bold}OLD_ENCRYPTION_KEY${colors.reset}   → Outgoing 64-hex symmetric encryption key`,
  );
  console.log(
    `  ${colors.bold}NEW_ENCRYPTION_KEY${colors.reset}   → Newly generated 64-hex symmetric encryption key\n`,
  );
...
function validateEnvironment() {
  ...
  if (!OLD_ENCRYPTION_KEY || !KEY_PATTERN.test(OLD_ENCRYPTION_KEY)) {
    printUsageAndExit(
      "OLD_ENCRYPTION_KEY must be exactly 64 hexadecimal characters",
    );
  }
  if (!NEW_ENCRYPTION_KEY || !KEY_PATTERN.test(NEW_ENCRYPTION_KEY)) {
    printUsageAndExit(
      "NEW_ENCRYPTION_KEY must be exactly 64 hexadecimal characters",
    );
  }
  ...
  for (const { contentCol, ivCol } of SENSITIVE_COLUMNS) {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          `${colors.yellow}⚠️ Warning: Record ID ${userRow.id} column '${contentCol}' failed decoding under both keys: ${fallbackErr.message}. Skipping.${colors.reset}`,
        );
        continue;
      }
    }
...
async function processUsersBatch(usersBatch, supabase, env, stats) {
  for (const userRow of usersBatch) {
    ...
    const { modified, updates } = processUserRecord(
      userRow,
      env.oldKeyBuffer,
      env.newKeyBuffer,
      stats,
...
async function executeRotation() {
  ...
  console.log(
    `${colors.green}✓ Credentials validated successfully.${colors.reset}`,
  );
  console.log(
    `Connecting to Supabase instance at: ${colors.cyan}${env.supabaseUrl}${colors.reset} ...\n`,
  );
  ...
  console.log(
    `${colors.bold}Commencing systematic database record inspection...${colors.reset}\n`,
  );
  ...
  while (hasMoreRecords) {
    ...
    if (fetchError) {
      console.error(
        `${colors.red}❌ Critical database traversal abort: ${fetchError.message}${colors.reset}`,
      );
      process.exit(1);
    }
  ...
  console.log(
    `  Total User Rows Scanned       : ${colors.bold}${stats.totalRowsTraversed}${colors.reset}`,
  );
  console.log(
    `  User Rows Successfully Upgraded: ${colors.bold}${colors.green}${stats.rowsModified}${colors.reset}`,
  );
  console.log(
    `  Total Symmetrical Fields Found: ${colors.bold}${stats.totalFieldsEncountered}${colors.reset}`,
  );
  console.log(
    `  Fields Decoded & Re-encrypted : ${colors.bold}${colors.green}${stats.fieldsReEncrypted}${colors.reset}`,
  );
  console.log(
    `  Fields Pre-migrated / Skipped : ${colors.bold}${colors.yellow}${stats.fieldsAlreadyUpgraded}${colors.reset}`,
  );
  if (stats.fieldsFailedDecryption > 0) {
    console.log(
      `  Fields Errored / Unresolved   : ${colors.bold}${colors.red}${stats.fieldsFailedDecryption}${colors.reset}`,
    );
  }
  console.log(
  ...
  console.log(
    `Update ${colors.bold}ENCRYPTION_KEY${colors.reset} inside your Infisical Dashboard \`/runtime\` folder to the new key string`,
  );
  console.log(
    `and restart your production container so the Infisical CLI wrapper injects the fresh key at boot.\n`,
  );
}
