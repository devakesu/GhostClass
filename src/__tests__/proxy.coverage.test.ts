...
vi.mock("@supabase/ssr", () => ({
  createServerClient: (...args: any[]) => {
    ...
    if (options?.cookies) {
      ...
      if (typeof options.cookies.getAll === "function") {
        options.cookies.getAll();
      }
      // Call setAll to cover the branch in proxy.ts
      if (typeof options.cookies.setAll === "function") {
...
describe("proxy.ts coverage hardening", () => {
  ...
  it("covers clearSessionCookies line 21 (sb- auth cookies)", async () => {
    ...
    expect(setCookies.some((c) => c.includes("sb-example-auth-token=;"))).toBe(
      true,
    );
  });
  ...
  it("covers EzyGo session self-healing success", async () => {
    ...
    const setCookies = response.headers.getSetCookie();
    expect(
      setCookies.some((c) => c.includes("ezygo_access_token=decrypted-token")),
    ).toBe(true);
  });

  it("covers EzyGo session self-healing failure (DB error)", async () => {
    mockGetUser.mockResolvedValue({
      data: { user: { id: "user-healing-fail" } },
      error: null,
    });
