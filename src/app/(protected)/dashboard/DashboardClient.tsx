...
const formatAcademicPeriod = (period: AcademicPeriod) =>
  `${period.semester.toUpperCase()} ${period.year}`;
...
function planAcademicShift(
  ...
  defaultAcademicInfo: {
    current_semester: AcademicSemester;
    current_year: string;
  },
): {
  ...
  const nextPeriod = shiftAcademicPeriod(
    effectiveSemester,
    effectiveYear,
    direction,
  );
  if (!nextPeriod) {
    return {
      next: null,
      errorMessage: "Could not compute the next academic period.",
    };
  }
  ...
  if (
    clampedNext.semester === effectiveSemester &&
    clampedNext.year === effectiveYear
  ) {
    return {
      next: null,
      infoMessage: "You cannot view past the maximum allowed academic period",
    };
  }
  ...
  if (wasClamped) {
    return {
      next: clampedNext,
      infoMessage: "Clamped to the nearest future academic period",
    };
  }
...
function useDashboardServerErrorToast(
  ...
  useEffect(() => {
    ...
    toast.error(
      isRateLimitError
        ? "EzyGo Rate Limit Reached"
        : "Dashboard Pre-fetch Failed",
      {
        description: isRateLimitError
          ? "Too many requests. Please wait."
          : "Failed to pre-load your data.",
        duration: 8000,
...
function useDashboardSyncFailureToast(
  syncSettled: boolean,
  syncFailed: boolean,
) {
  ...
  useEffect(() => {
    ...
    toast.warning("Background sync delayed", {
      description:
        "Showing cached data. Latest attendance updates will appear when sync recovers.",
    });
...
const getActiveCourseStats = (
  ...
  const noDataCount = countNoDataCodes(
    catalogCodes,
    activeCodes,
    disabledWithDataCodes,
    disabledCodes,
  );
...
export default function DashboardClient({
  ...
  const [pendingChange, setPendingChange] = useState<AcademicPeriod | null>(
    null,
  );
  ...
  const { data: rawClassCourses, isFetching: isFetchingClassCourses } =
    useFetchClassCourses({
      semester: currentSem,
      year: currentYear,
      enabled: isProfileReady && !!currentSem && !!currentYear &&
  ...
  } = useAllCourseDetails(courseList, currentSem, currentYear, {
  ...
  const pendingPeriodLabel = pendingChange
    ? formatAcademicPeriod(pendingChange)
    : null;
    ...
    if (attendanceData) {
      return (
        <AttendanceCalendar
          ...
          coursesData={{
            courses: courseRegistry as unknown as Record<string, Course>,
          }}
  ...
  if (!profile || isLoadingProfile || !currentSem || !currentYear) {
    return <CompLoading />;
  }
        ...
        <AnimatePresence>
          {isGlobalLoading && (
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="fixed inset-0 z-100 flex flex-col items-center justify-center bg-background/60 backdrop-blur-md transition-all duration-300"
            >
        ...
        <main className="flex-1 container mx-auto px-4 md:px-6 pt-4 md:pt-6">
          {serverError && (
            <div className="mb-4">
              <div
                role="alert"
                className="w-full flex items-center justify-between gap-4 p-3 rounded-md border bg-amber-50 border-amber-200 text-amber-800"
              >
                <div className="text-sm">
                  ...
                  <div className="text-xs mt-0.5">
                    {isRateLimitError
                      ? "Too many requests. Some data may be unavailable."
                      : "Failed to preload dashboard data. You can retry or continue."}
                  </div>
                </div>
                <div className="shrink-0">
                  <button
                    ...
                    className="inline-flex items-center px-3 py-1.5 rounded-md bg-amber-600 text-white text-sm font-semibold"
                  >
                    Retry
                  </button>
                </div>
          ...
          <div className="mb-6 flex flex-col lg:flex-row gap-6 lg:items-end justify-between">
            <div className="flex flex-col gap-3 flex-1">
              ...
              <p className="text-xs text-foreground/50 font-medium italic">
                For students juggling classes, internals, labs, submissions,
                caffeine, and “I’ll study tomorrow” energy ☕📚
              </p>
