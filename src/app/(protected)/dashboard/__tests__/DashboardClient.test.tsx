  ...
  CourseGrid: ({
    ...
    <div data-testid="course-grid">
      ...
      {sortedCourses.map((c) => (
        <button
          key={c.id}
          onClick={() => onEditInstructor?.(c, "Instructor", false, null)}
        >
          Edit {c.code}
        </button>
      ))}
...
describe("DashboardClient", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(syncHooks.useSyncOnMount).mockReturnValue({
      isSyncing: false,
      syncSettled: true,
      syncFailed: false,
    });
  ...
  it("triggers onEditInstructor from CourseGrid", async () => {
    vi.mocked(coursesHooks.useFetchCourses).mockReturnValue({
      data: {
        courses: { "1": { id: 1, code: "CS101", name: "Computer Science" } },
      },
      isLoading: false,
      isFetching: false,
      isError: false,
