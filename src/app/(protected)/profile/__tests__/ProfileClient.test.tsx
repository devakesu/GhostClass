...
describe('ProfileClient', () => {
  ...
  it('shows error toast when profile ID is missing during upload', () => {
    vi.mocked(useProfile).mockReturnValue({ data: { ...mockProfile, id: null }, isLoading: false } as any);
    render(<ProfileClient />);
    ...
    expect(toast.error).toHaveBeenCalledWith(expect.stringContaining("verify your profile"));
  });
  ...
  it('compresses large images before upload', async () => {
    ...
    const compressedFile = new File([new Uint8Array(1 * 1024 * 1024)], 'large.jpg', { type: 'image/jpeg' });
    ...
    await waitFor(() => {
      expect(toast.info).toHaveBeenCalledWith(expect.stringContaining("Compressing"), expect.anything());
  ...
  it('handles compression error by uploading original and warning user', async () => {
    ...
    await waitFor(() => {
      expect(toast.warning).toHaveBeenCalledWith(expect.stringContaining("Could not compress"));
      expect(uploadUserAvatar).toHaveBeenCalledWith(largeFile);
    });
  });

  it('falls back to username if name is missing', () => {
    vi.mocked(useProfile).mockReturnValue({ data: { ...mockProfile, first_name: '', last_name: '' }, isLoading: false } as any);
    render(<ProfileClient />);
    expect(screen.getByText('testuser')).toBeInTheDocument();
  });
  ...
  it('renders fallback join date if ezygo_created_at is missing', () => {
    vi.mocked(useProfile).mockReturnValue({ 
      data: { ...mockProfile, ezygo_created_at: null, created_at: '2022-05-20T00:00:00Z' }, 
      isLoading: false 
    } as any);
    render(<ProfileClient />);
  ...
  it('renders loading overlay during upload', async () => {
    vi.mocked(useProfile).mockReturnValue({ data: mockProfile, isLoading: false } as any);
    vi.mocked(uploadUserAvatar).mockReturnValue(new Promise(resolve => setTimeout(() => resolve('url'), 100)));
