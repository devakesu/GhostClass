...
export default function TrackingClient() {
  ...
  const filteredCourseKeys = useMemo(() => {
    ...
    return allCourseKeys.filter((k) => k === selectedCourseFilter);
  }, [
    allCourseKeys,
  ...
  const deleteAllTrackingData = async () => {
    ...
    try {
      ...
      if (selectedCourseFilter !== "all") {
        ...
        if (course) {
          keys.add(course.id.toString());
          if (course.code) keys.add(course.code);
        }

        query = query.in("course", Array.from(keys));
      }
  ...
  if (
    isInitialLoading && !isTrackingError && !isCountError && !isCoursesError &&
    !isAttendanceError && !isSemesterError && !isAcademicYearError
  ) {
    return <Loading />;
  }

  // Note: Regional error cases (isTrackingError, etc.) are now handled globally
                        ...
                        <Badge className="text-sm py-1 px-3 bg-yellow-500/12 text-yellow-600 dark:text-yellow-400 border border-yellow-500/40 dark:border-yellow-500/20">
                          {selectedCourseFilter === "all"
                            ...
                            : (
                              <>
                                <strong>
                                  {groupedAllData[selectedCourseFilter]?.length}
                                </strong>{" "}
                ...
                <div className="relative mx-auto flex w-full max-w-175 flex-col gap-4 overflow-visible">
                  ...
                  >
                    {currentCourseKeys.map((courseName) => {
                      ...
                      const statusGroups: Record<StatusKey, typeof items> = {
                        Present: [],
                        "Duty Leave": [],
                        Absent: [],
                      };
                                ...
                                >
                                  ...
                                  <div className="flex flex-col gap-3">
                                    {groupItems.map((trackingItem) => {
                                      ...
                                      // Status Logic
                                      const isCorrection =
                                        trackingItem.status === "correction";
                                      const attCode = Number(
                                        trackingItem.attendance,
                                      );
                                      ...
                                      if (isCorrection) {
                                        ...
                                        if (officialSession) {
                                          const offCode = Number(
                                            officialSession.attendance,
                                          );
                                          if (offCode === 110) {
                                      ...
                                      if (isCorrection) {
                                        if (userColor === "orange") {
                                          ...
                                          cardBgClass =
                                            "bg-red-500/5 border-red-500/35 dark:border-red-500/20";
                                        } else {
                                          cardBgClass =
                                            "bg-green-500/5 border-green-500/35 dark:border-green-500/20";
                                          statusBadgeClass =
                          ...
                          {hasMore && (
                            ...
                            >
                              {isExpanded
                                ? `Show Less`
                                : `Show ${
                                  items.length - recordsPerCourseInitial
                                } More ${
                                  items.length - recordsPerCourseInitial === 1
                                    ? "Record"
                                    : "Records"
                                }`}
                            </m.button>
