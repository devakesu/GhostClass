...
async function prepareRequestBody(req: NextRequest, method: string, decryptedBody?: unknown) {
  if (method === "GET" || method === "HEAD") return { body: undefined, contentType: "application/json" };
  
  if (decryptedBody) return { body: JSON.stringify(decryptedBody), contentType: "application/json" };
  ...
  const resolvedType = contentType.split(";")[0].trim().toLowerCase() || "application/octet-stream";
...
function validateProxyRequestPath(path?: string[]): { fullPath: string; errorResponse: NextResponse | null } {
  if (!BASE_API_URL) {
    return { fullPath: "", errorResponse: NextResponse.json({ message: "Backend URL not configured" }, { status: 500 }) };
  }
  if (!path || path.length === 0) {
    return { fullPath: "", errorResponse: NextResponse.json({ message: "Missing path" }, { status: 400 }) };
  }
...
async function validateProxyAuthAndOrigin(
  ...
  const originError = await validateOrigin(req, fullPath, isPublic, isMobileApp);
  if (originError) {
    return { errorResponse: NextResponse.json({ message: originError.message }, { status: originError.status }) };
  }
...
function handleProxyUpstreamError(err: unknown, lastAttemptedEgressName: string): NextResponse {
  const egressHeader = { "x-egress-target": lastAttemptedEgressName };
  const errObj = err as { name?: string; status?: number; body?: string; headers?: unknown };
  
  if (errObj?.name === "CircuitBreakerOpenError") {
    return NextResponse.json({ message: "EzyGo issues." }, { status: 503, headers: egressHeader });
  }
...
async function handleBatchedEgress(
  ...
  try {
    ...
    return NextResponse.json(data, { status: 200, headers: { "x-egress-mode": "batched" } });
  } catch (err) {
...
async function attemptSingleTarget(
  ...
  try {
    const init = buildEgressRequestInit(method, baseHeaders, body, egress.secret, controller.signal);
    const res = await fetch(`${egress.baseUrl}/${fullPath}${req.nextUrl.search}`, init);
    ...
    if (RETRYABLE_UPSTREAM_STATUSES.has(res.status)) {
      throw new UpstreamServerError(`Err: ${res.status}`, res.status, res.statusText, text, res.headers);
    }
...
async function executeCircuitBreakerLoop(
  ...
  for (let i = 0; i < EGRESS_TARGETS.length; i++) {
    ...
    try {
      return await attemptSingleTarget(egress, req, method, fullPath, baseHeaders, body);
    } catch (err) {
...
async function forward(
  ...
  const { body, contentType: resolvedContentType } = await prepareRequestBody(req, method, decryptedBody);

  try {
    const isBatchablePost = method === "POST" && (!body || body === "{}");
    const canBatch = (method === "GET" || isBatchablePost) && !isPublic && token && process.env.VITEST !== "true";
    
    if (canBatch) {
      const batchedRes = await handleBatchedEgress(req, method, fullPath, token, isBatchablePost, clientIp, clientUserAgent);
    ...
    const result = await ezygoCircuitBreaker.execute(async () => {
      ...
      return await executeCircuitBreakerLoop(
        ...
        (name) => { lastAttemptedEgressName = name; }
      );
    });
    ...
    if (!result.res.ok) {
      ...
      return NextResponse.json({ message: msg, status: result.res.status }, { status: result.res.status, headers: { ...sanitizedHeaders, "x-egress-target": result.egressName } });
    }

    return new NextResponse(result.text, { status: result.res.status, headers: { ...sanitizedHeaders, "x-egress-target": result.egressName } });
...
export const POST = withSecurity(async (req, { params, decryptedBody, authType }) => {
  ...
  if (res.ok && (path.join("/").includes("default_semester") || path.join("/").includes("default_academic_year"))) {
