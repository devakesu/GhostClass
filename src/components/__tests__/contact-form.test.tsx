  ...
  default: ({ onVerify, onError, onExpire }: any) => (
    <div>
      ...
      <div
        ...
        onClick={() => onError(new Error("Turnstile error"))}
      >
        Error
      </div>
      <div data-testid="turnstile-expire" onClick={() => onExpire()}>
        Expire
      </div>
    </div>
  ),
...
describe("ContactForm", () => {
  ...
  it("handles Turnstile expiration", async () => {
    ...
    expect(
      await screen.findByRole("button", { name: /Waiting for Verification/i }),
    ).toBeInTheDocument();
  });
