...
describe('AddRecordTrigger', () => {
  ...
  it('renders the trigger button', () => {
    render(<AddRecordTrigger user={mockUser as any} onSuccess={mockOnSuccess} />);
  ...
  it('handles success correctly', async () => {
    ...
    await waitFor(() => {
      expect(invalidateQueries).toHaveBeenCalledWith({ queryKey: ["attendance-report"] });
      expect(invalidateQueries).toHaveBeenCalledWith({ queryKey: ["attendance-report-all"] });
      expect(invalidateQueries).toHaveBeenCalledWith({ queryKey: ["track_data"] });
  ...
  it('handles user object without auth_id', () => {
    ...
    render(<AddRecordTrigger user={userNoAuth as any} onSuccess={mockOnSuccess} />);
    fireEvent.click(screen.getByRole('button', { name: /Add new record/i }));
    expect(screen.getByTestId('attendance-dialog')).toBeInTheDocument();
  });
});
