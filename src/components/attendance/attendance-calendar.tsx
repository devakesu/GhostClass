...
import { AttendanceEvent, AttendanceReport, Course } from "@/types";
...
const getNormalizedSession = (s: string | number) =>
  parseInt(normalizeSession(s), 10) || 0;
...
function normalizeTrackerRecords(
  ...
  return trackingData.map((item) => {
    ...
    return {
      ...t,
      _isoDate: normalizeToISODate(t.date),
...
function resolveCourseCode(
  ...
  const course = Object.values(coursesData?.courses || {}).find(
    (c) =>
      String(c.id) === id ||
      (c.code && normalizeCourseCode(c.code) === normalizedInput),
  );
  if (course?.code) {
  ...
  const custom = classCourses?.find(
    (cc) => normalizeCourseCode(cc.course_code) === normalizedInput,
  );
  if (custom) {
...
function resolveCourseName(
  ...
  const custom = classCourses?.find(
    (cc) => normalizeCourseCode(cc.course_code) === normalizedId,
  );
  if (custom) {
...
function resolveSessionName(
  ...
  if (
    !sessionNameStr ||
    sessionNameStr === "null" ||
    isNumericId(sessionNameStr)
  ) {
    /* eslint-disable security/detect-object-injection */
    const regSession = sessionNameStr
      ? attendanceData.sessions?.[sessionNameStr]
      : undefined;
  ...
  Object.entries(attendanceData.studentAttendanceData).forEach(
    ([dateStr, sessions]) => {
      ...
      Object.entries(sessions).forEach(
        ([sessionKey, sessionDataObj], index) => {
          const sessionData = sessionDataObj as {
...
function computeExtraMergedEvents(
  ...
  normalizedTrackingData.forEach((t) => {
    ...
    if (t._isoDate === dbDateStr) {
      ...
      const alreadyMerged = processedEvents.some(
        (ev) => generateSlotKey(ev.courseId, ev.date, ev.sessionName) === key,
      );
...
function mergeSelectedDateEvents(
  ...
  return merged.sort(
    (a, b) =>
      getNormalizedSession(a.sessionName) - getNormalizedSession(b.sessionName),
  );
}
...
function findDefaultSelectedDate(
  ...
  if (attendanceData?.studentAttendanceData) {
    const dates = Object.keys(attendanceData.studentAttendanceData).sort(
...
function computeCellClassName(
  ...
  if (isOutOfRange) {
    return (
      baseClass + "opacity-20 cursor-not-allowed pointer-events-none grayscale"
    );
  }
  ...
  if (isSelected) {
    ...
    baseClass +=
      "bg-red-500/20 text-red-500 dark:text-red-400 hover:bg-red-500/30 border border-red-500/50 dark:border-red-500/30";
  } else if (status === "otherLeave") {
    baseClass +=
      "bg-blue-500/20 text-blue-500 dark:text-blue-400 hover:bg-blue-500/30 border border-blue-500/50 dark:border-blue-500/30";
  } else if (status === "dutyLeave") {
    baseClass +=
      "bg-yellow-500/20 text-yellow-600 dark:text-yellow-400 hover:bg-yellow-500/30 border border-yellow-500/50 dark:border-yellow-500/30";
  } else if (status === "present") {
    baseClass +=
      "bg-emerald-500/20 text-emerald-500 dark:text-emerald-400 hover:bg-emerald-500/30 border border-emerald-500/50 dark:border-emerald-500/30";
  } else if (status === "normal") {
    baseClass +=
      "bg-indigo-500/20 text-indigo-500 dark:text-indigo-400 hover:bg-indigo-500/30 border border-indigo-500/50 dark:border-indigo-500/30";
  } else if (status !== null) {
  ...
  if (isTodayLocal) {
    baseClass +=
      " ring-2 ring-offset-1 ring-offset-background ring-violet-600 dark:ring-violet-500";
  }
  return baseClass;
}
  ...
  return (
    <div className="shrink-0 w-full sm:w-auto flex items-center justify-end gap-2">
      {event.isCorrection && (
        <Badge
          variant="outline"
          className="text-[10px] h-6 px-2 bg-orange-500/10 text-orange-600 dark:text-orange-400 border-orange-500/40 dark:border-orange-500/25 gap-1.5"
        >
          <AlertTriangle className="w-3 h-3" aria-hidden="true" />
          Official: {event.originalStatus}
        </Badge>
      )}
      <Button
        variant="ghost"
...
function RenderSelfMarkedActions({
  ...
  return (
    <div className="shrink-0 w-full sm:w-auto flex items-center justify-end gap-2">
      <Badge
        variant="outline"
        className="text-[10px] h-6 px-2 bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border-indigo-500/40 dark:border-indigo-500/25 gap-1.5"
      >
...
export function AttendanceCalendar({
  ...
  useEffect(() => {
    if (academicRange && semester && year && prevTermKey !== currentTermKey) {
      const initDate = determineInitialDate(
        academicRange,
        attendanceData,
        trackingData,
      );
  ...
  const handleWriteTracking = async (
    ...
    try {
      ...
      Sentry.captureException(error, {
        tags: {
          type: "tracking_add_error",
          location: "AttendanceCalendar/handleWriteTracking",
        },
        extra: {
          courseId,
          dateStr,
  ...
  const getFirstDayOfMonth = useCallback(
    (year: number, month: number) => new Date(year, month, 1).getDay(),
    [],
  );
  ...
  const yearOptions = useMemo(() => {
    const baseYears = Array.from(
      {
        length: new Date().getFullYear() + 1 - 2018 + 1,
      },
      (_, i) => 2018 + i,
    );
  ...
  const calendarCells = useMemo(() => {
    ...
    const dayCells = Array(daysInMonth)
      ...
      .map((_, index) => {
        ...
        const isOutOfRange = academicRange
          ? date < academicRange.min || date > academicRange.max
          : false;
        const isTodayLocal = isToday(date);

        const className = computeCellClassName(
          status,
          isSelected,
          isOutOfRange,
          isTodayLocal,
        ...
        const dateLabel = `${monthNames[date.getMonth()]} ${
          index + 1
        }, ${date.getFullYear()}${selectSuffix}${statusSuffix}`;
        ...
        <Card className="border border-border/40 custom-container">
          <CardContent className="p-6">
            <div className="text-muted-foreground text-sm flex items-center gap-2">
              <Loader2 className="h-4 w-4 animate-spin" /> Waking up EzyGo...
            </div>
          </CardContent>
        </Card>
      </div>
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
      <Card className="overflow-hidden border border-border/40 custom-container h-full flex flex-col">
        <CardHeader className="pb-2 flex flex-row flex-wrap items-center justify-center sm:justify-between gap-2 border-b border-border/40">
          <div className="flex items-center gap-2 max-sm:contents">
            <Select value={filter} onValueChange={setFilter}>
              <SelectTrigger
                className="w-32.5 h-9 bg-background/60 border-border/60 text-sm capitalize custom-dropdown"
            ...
            >
              ...
              >
                <SelectValue>
                  {monthOptions[currentDate.month].name}
                </SelectValue>
              </SelectTrigger>
              <SelectContent className="bg-background/90 border-border/60 backdrop-blur-md custom-dropdown max-h-70">
            ...
            >
              <SelectTrigger
                className="w-22.5 h-9 bg-background/60 border-border/60 text-sm custom-dropdown"
        ...
        <CardContent className="p-4 flex-1 flex flex-col h-full">
          ...
          <div className="flex flex-wrap gap-4 mt-6 text-muted-foreground text-xs justify-center border-t border-border/40 pt-4 shrink-0">
            ...
            <div className="flex items-center gap-2">
              <div className="h-3 w-3 rounded-full ring-2 ring-violet-600 dark:ring-violet-500 ring-offset-1" />
              <span>today</span>
            </div>
          </div>
        </CardContent>
      ...
      <Card className="overflow-hidden border-border/40 shadow-sm bg-card/50 flex flex-col h-full">
        <CardHeader className="border-b border-border/40 py-4 px-6 bg-muted/20">
          <CardTitle className="text-sm flex items-center justify-between font-semibold">
            ...
            <Badge
              variant="secondary"
              className="font-normal text-xs bg-background/80"
              aria-label={`${selectedDateEvents.length} attendance sessions`}
            >
      ...
      <Dialog
        ...
        onOpenChange={(open) => {
          if (!open) handleRemarkCancel();
        }}
      >
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            <DialogTitle>
              {pendingRemarkAction?.title || "Add Remark"}
            </DialogTitle>
            <DialogDescription>
              {pendingRemarkAction?.description ||
                "Enter a remark for this action."}
            </DialogDescription>
          </DialogHeader>
          <div className="py-2">
            <Label htmlFor="remark-calendar" className="text-sm mb-1.5 block">
              Remark (Optional)
            </Label>
            <Input
              id="remark-calendar"
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") handleRemarkConfirm();
              }}
              autoFocus
            />
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={handleRemarkCancel}>
              Cancel
            </Button>
            <Button
