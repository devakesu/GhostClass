  ...
  useEffect(() => {
    if (process.env.NODE_ENV === "development") {
      if (donateUrlRaw && !donateUrl) {
        logger.warn(`[Footer] Invalid Donate URL: ${donateUrlRaw}`);
      }
      if (githubUrlRaw && !githubUrl) {
        logger.warn(`[Footer] Invalid GitHub URL: ${githubUrlRaw}`);
      }
      if (authorUrlRaw && !authorUrl) {
        logger.warn(`[Footer] Invalid Author URL: ${authorUrlRaw}`);
      }
    }
  }, [
    donateUrlRaw,
          ...
          <div className="flex items-center gap-2">
            {donateUrl && (
              ...
              >
                ...
                >
                  <Coffee
                    className="w-3.5 h-3.5 mr-2 text-muted-foreground group-hover:text-pink-500 transition-colors"
                    aria-hidden="true"
                  />
            ...
            >
              ...
              >
                <Star
                  className="w-3.5 h-3.5 mr-2 text-muted-foreground group-hover:text-yellow-500 transition-colors"
                  aria-hidden="true"
                />
