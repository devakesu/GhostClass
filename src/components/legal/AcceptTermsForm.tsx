...
export function AcceptTermsForm() {
  ...
  return (
    <div className="border border-border/50 bg-card/80 backdrop-blur-xl shadow-2xl rounded-lg overflow-hidden">
      {/* Header */}
      ...
      <div className="px-6 py-2">
        <div className="w-full rounded-lg border border-border bg-muted/50 p-5">
      ...
      <div className="p-6 pt-2 flex flex-col gap-4">
        <label className="flex items-center space-x-3 rounded-lg border border-border bg-muted/50 p-3 hover:bg-muted/80 transition-colors cursor-pointer">
          <Checkbox
            id="terms"
            checked={checked}
          ...
          <span id="terms-label" className="text-xs text-muted-foreground font-normal select-none">
            ...
            <Link
              href="/legal"
              className="text-foreground hover:underline hover:text-purple-600 dark:hover:text-purple-400 transition-colors"
              onClick={(e) => {
                // Prevent the label from toggling the checkbox when link is clicked.
                // Navigate within the PWA (same window) so pressing back returns
