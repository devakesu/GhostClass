...
describe('DeleteAccount Component', () => {
  ...
  it('handles deletion failure', async () => {
    mockSupabase.auth.getUser.mockResolvedValue({ data: { user: { id: 'user123' } } });
    mockSupabase.rpc.mockResolvedValue({ error: { message: 'Failed to delete' } });
  ...
  it('handles storage listing error gracefully', async () => {
    mockSupabase.auth.getUser.mockResolvedValue({ data: { user: { id: 'user123' } } });
    mockSupabase.storage.list.mockResolvedValue({ data: null, error: { message: 'List failed' } });
    mockSupabase.rpc.mockResolvedValue({ error: null });
    ...
    await waitFor(() => {
      expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('Failed to list'), expect.any(Object));
  ...
  it('handles storage pagination correctly', async () => {
    ...
    mockSupabase.storage.list
      .mockResolvedValueOnce({ data: Array(100).fill({ name: 'file.png' }), error: null })
    ...
    await waitFor(() => {
      ...
      expect(mockSupabase.storage.remove).toHaveBeenCalledWith(expect.arrayContaining(['user123/file.png']));
      expect(mockSupabase.rpc).toHaveBeenCalled();
    });
  });
  ...
  it('handles storage removal error gracefully', async () => {
    mockSupabase.auth.getUser.mockResolvedValue({ data: { user: { id: 'user123' } } });
    mockSupabase.storage.list.mockResolvedValue({ data: [{ name: 'file.png' }], error: null });
    mockSupabase.storage.remove.mockResolvedValue({ error: { message: 'Remove failed' } });
    mockSupabase.rpc.mockResolvedValue({ error: null });
    const { logger } = await import('@/lib/logger');
    ...
    await waitFor(() => {
      expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('Failed to remove'), expect.any(Object));
      expect(mockSupabase.rpc).toHaveBeenCalled();
    });
  });
  ...
  it('handles storage abort error specifically', async () => {
    ...
    await waitFor(() => {
      expect(logger.warn).toHaveBeenCalledWith(expect.stringContaining('aborted'), expect.any(Object));
      expect(mockSupabase.rpc).toHaveBeenCalled();
    });
  });
});
