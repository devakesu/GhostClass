...
describe('settings hooks', () => {
  ...
  describe('useSetSemester', () => {
    it('updates semester and invalidates queries', async () => {
      ...
      expect(axios.post).toHaveBeenCalledWith('/user/setting/default_semester', { default_semester: 'odd' });
  ...
  describe('useSetAcademicYear', () => {
    it('updates academic year successfully', async () => {
      ...
      expect(axios.post).toHaveBeenCalledWith('/user/setting/default_academic_year', { default_academic_year: '2024-25' });
  ...
  describe('useFetchUserSettings', () => {
    it('combines semester and academic year queries', async () => {
      vi.mocked(axios.get).mockImplementation((url: any) => {
        if (url === '/user/setting/default_semester') return Promise.resolve({ data: 'even' });
        if (url === '/user/setting/default_academic_year') return Promise.resolve({ data: '2023-24' });
      ...
      expect(result.current.data).toEqual({ semester: 'even', academicYear: '2023-24' });
    });
  });
});
