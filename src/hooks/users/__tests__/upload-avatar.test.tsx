...
describe("uploadUserAvatar", () => {
  ...
  it("should upload avatar and update profile successfully", async () => {
    ...
    storageFrom.mockReturnValue({
        ...
        getPublicUrl: vi.fn().mockReturnValueOnce({ data: { publicUrl: "https://test.supabase.co/storage/v1/object/public/avatars/user123/123.png" } }),
  ...
  it("should throw if not authenticated", async () => {
    ...
    await expect(uploadUserAvatar(mockFile)).rejects.toThrow("User not authenticated");
    expect(Sentry.captureException).toHaveBeenCalled();
  });

  it("should throw if file type unsupported", async () => {
    mockSupabase.auth.getUser.mockResolvedValueOnce({ data: { user: mockUser }, error: null });
    const invalidFile = new File(["test"], "test.svg", { type: "image/svg+xml" });

    await expect(uploadUserAvatar(invalidFile)).rejects.toThrow("Unsupported file type");
  });
  ...
  it("should throw if storage upload fails", async () => {
    ...
    const storageFrom = mockSupabase.storage.from as any;
    storageFrom.mockReturnValue({
        upload: vi.fn().mockResolvedValueOnce({ error: { message: "Quota exceeded" } }),
    });

    await expect(uploadUserAvatar(mockFile)).rejects.toThrow("Upload failed: Quota exceeded");
  });
  ...
  it("should throw if public URL origin mismatch", async () => {
    ...
    storageFrom.mockReturnValue({
        upload: vi.fn().mockResolvedValueOnce({ error: null }),
        getPublicUrl: vi.fn().mockReturnValueOnce({ data: { publicUrl: "https://malicious.com/fake.png" } }),
    });

    await expect(uploadUserAvatar(mockFile)).rejects.toThrow("Avatar URL origin mismatch");
  });
  ...
  it("should throw if profile update fails and attempt cleanup", async () => {
    ...
    storageFrom.mockReturnValue({
        upload: vi.fn().mockResolvedValueOnce({ error: null }),
        getPublicUrl: vi.fn().mockReturnValueOnce({ data: { publicUrl: "https://test.supabase.co/1.png" } }),
        remove: vi.fn().mockResolvedValueOnce({ error: null }),
    });
    ...
    await expect(uploadUserAvatar(mockFile)).rejects.toThrow("Profile update failed");
    expect(mockSupabase.storage.from).toHaveBeenCalledWith("avatars");
  ...
  it("should perform background cleanup of old avatars", async () => {
      ...
      storageFrom.mockReturnValue({
          upload: vi.fn().mockResolvedValueOnce({ error: null }),
          getPublicUrl: vi.fn().mockReturnValueOnce({ data: { publicUrl: "https://test.supabase.co/1.png" } }),
          list: vi.fn().mockResolvedValueOnce({ data: [{ name: "old.png" }], error: null }),
          remove: vi.fn().mockResolvedValueOnce({ error: null }),
      });
      ...
      expect(storageFrom().list).toHaveBeenCalledWith(mockUser.id, expect.any(Object), expect.any(Object));
      expect(storageFrom().remove).toHaveBeenCalledWith([`${mockUser.id}/old.png`]);
  });
  ...
  it("should handle background cleanup failure gracefully", async () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith(expect.stringContaining("Background cleanup failed"), expect.any(Error));
      expect(Sentry.captureException).toHaveBeenCalled();
  });
  ...
  it("should skip cleanup if no old files exist", async () => {
      ...
      storageFrom.mockReturnValue({
          upload: vi.fn().mockResolvedValueOnce({ error: null }),
          getPublicUrl: vi.fn().mockReturnValueOnce({ data: { publicUrl: "https://test.supabase.co/1.png" } }),
          list: vi.fn().mockResolvedValueOnce({ data: [{ name: expectedFileName }], error: null }),
          remove: vi.fn(),
      });
