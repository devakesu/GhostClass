...
describe("CircuitBreaker", () => {
  ...
  it("fails fast when OPEN", async () => {
    ...
    const fn2 = vi.fn();
    await expect(ezygoCircuitBreaker.execute(fn2)).rejects.toThrow(
      CircuitBreakerOpenError,
    );
  ...
  it("NonBreakerError counts as success in HALF_OPEN", async () => {
    ...
    await expect(ezygoCircuitBreaker.execute(fn404)).rejects.toThrow(
      NonBreakerError,
    );
    expect((await ezygoCircuitBreaker.getStatus()).successCount).toBe(1);
  });
  ...
  it("UpstreamServerError carries status and body", () => {
    const error = new UpstreamServerError(
      "Server Error",
      502,
      "Bad Gateway",
      "Error Body",
  ...
  it("handles non-Error failures", async () => {
    await expect(
      ezygoCircuitBreaker.execute(() => Promise.reject("string error")),
