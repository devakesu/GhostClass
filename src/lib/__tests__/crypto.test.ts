...
import { __resetCachedKey, decrypt, encrypt } from "../crypto";
...
describe("crypto.ts", () => {
  ...
  it("throws error for invalid IV format in decrypt", () => {
    expect(() => decrypt({ iv: "invalid-iv", content: "tag:content" })).toThrow(
      "Invalid IV format",
    );
  });
  ...
  it("throws error for invalid content format (missing separator) in decrypt", () => {
    ...
    expect(() => decrypt({ iv, content: "no-separator" })).toThrow(
      "Invalid content format (missing separator)",
    );
  });

  it("throws error for invalid content format (too many separators) in decrypt", () => {
    const iv = "0".repeat(24);
    expect(() => decrypt({ iv, content: "tag:content:extra" })).toThrow(
      "Invalid content format (unexpected separators)",
    );
  });

  it("throws error for non-hex characters in decrypt", () => {
    const iv = "0".repeat(24);
    expect(() => decrypt({ iv, content: "tag:content-with-non-hex!" })).toThrow(
      "Invalid content format (non-hex characters)",
    );
  });
  ...
  it("throws error for missing IV or content in decrypt", () => {
    ...
    expect(() => decrypt({ iv: "", content: "" })).toThrow(
      "Invalid input: IV and content are required",
    );
