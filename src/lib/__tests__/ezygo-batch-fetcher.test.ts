...
describe("ezygo-batch-fetcher", () => {
  ...
  it("evicts from cache on transient failure", async () => {
    ...
    await expect(fetchEzygoData("/fail", "token")).rejects.toThrow(
      "EzyGo API error",
    );
  ...
  it("does NOT evict from cache on NonBreakerError", async () => {
    ...
    await expect(fetchEzygoData("/404", "token")).rejects.toThrow(
      NonBreakerError,
    );
  ...
  it("throws if no egress target is configured", async () => {
    ...
    await expect(fetchEzygoData("/test", "token")).rejects.toThrow(
      "No egress target configured",
    );
  });
