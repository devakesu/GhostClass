...
describe("validate-env utility", () => {
  ...
  it("warns if HOSTNAME=0.0.0.0 in production with local domain", () => {
    ...
    expect(console.warn).toHaveBeenCalledWith(
      expect.stringContaining("SECURITY: HOSTNAME=0.0.0.0"),
    );
  });
  ...
  describe("Core Security and Infrastructure", () => {
    ...
    it("fails if mandatory variables are missing", () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("ENCRYPTION_KEY is required"),
      );
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("CRON_SECRET is required"),
      );
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("NEXT_PUBLIC_SUPABASE_URL is required"),
      );
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("NEXT_PUBLIC_APP_NAME is required"),
      );
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("MIN_APP_VERSION is required"),
      );
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("NEXT_PUBLIC_APP_URL is required"),
      );
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("NEXT_PUBLIC_APP_DOMAIN is required"),
      );
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("NEXT_PUBLIC_APP_EMAIL is required"),
      );
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("NEXT_PUBLIC_BACKEND_URL is required"),
      );
    });
  ...
  describe("Optional Proxies and Sentry", () => {
    it("validates NEXT_PUBLIC_APP_URL parse error", () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("must be a valid absolute URL"),
      );
    });
    ...
    it("validates NEXT_PUBLIC_APP_DOMAIN invalid characters", () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("contains invalid hostname characters"),
      );
    });
    ...
    it("validates NEXT_PUBLIC_APP_DOMAIN bare domain check", () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("must be a bare domain without path"),
      );
    });
    ...
    it("validates CF_PROXY_URL parse error", () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("CF_PROXY_URL must be a valid absolute URL"),
      );
    });
    ...
    it("validates AWS_SECONDARY_URL and secret", () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining(
          "AWS_SECONDARY_URL must use http or https protocol",
        ),
      );
    });
    ...
    it("validates AWS_SECONDARY_URL parse error", () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining(
          "AWS_SECONDARY_URL must be a valid absolute URL",
        ),
      );
    });
    ...
    it("validates AWS_SECONDARY_SECRET missing", () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("AWS_SECONDARY_SECRET is required"),
      );
    });
    ...
    it("validates AWS_SECONDARY_SECRET short", () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("AWS_SECONDARY_SECRET is too short"),
      );
    });
    ...
    it("validates Supabase dev keys format", () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("DEV_PUBLISHABLE_KEY looks invalid"),
      );
      ...
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining(
          "DEV_SECRET_KEY/SUPABASE_SERVICE_ROLE_KEY looks invalid",
        ),
      );
    });
    ...
    it("errors if SENTRY_HASH_SALT is missing in production", () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("SENTRY_HASH_SALT is required in production"),
      );
    });
    ...
    it("warns if SENTRY_HASH_SALT is missing in development", () => {
      ...
      validateEnvironment();
      expect(console.warn).toHaveBeenCalledWith(
        expect.stringContaining("using development-only fallback"),
      );
    });
    ...
    it("warns if APP_COMMIT_SHA is missing in production", () => {
      ...
      validateEnvironment();
      expect(console.warn).toHaveBeenCalledWith(
        expect.stringContaining("APP_COMMIT_SHA is not set"),
      );
    });
    ...
    it("validates GA ID and API Secret strictly", () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("NEXT_PUBLIC_GA_ID appears invalid"),
      );
      ...
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("GA_API_SECRET is required"),
      );
      ...
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("GA_API_SECRET appears invalid"),
      );
    });
    ...
    it("validates rate limit numeric ranges strictly", () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("SYNC_RATE_LIMIT_REQUESTS is invalid"),
      );
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("SYNC_RATE_LIMIT_WINDOW is invalid"),
      );
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("CONTACT_RATE_LIMIT_REQUESTS is invalid"),
      );
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("CONTACT_RATE_LIMIT_WINDOW is invalid"),
      );
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("AUTH_RATE_LIMIT_REQUESTS is invalid"),
      );
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("AUTH_RATE_LIMIT_WINDOW is invalid"),
      );
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("PROXY_RATE_LIMIT_REQUESTS is invalid"),
      );
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("PROXY_RATE_LIMIT_WINDOW is invalid"),
      );
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("RATE_LIMIT_REQUESTS is invalid"),
      );
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("RATE_LIMIT_WINDOW is invalid"),
      );
    });
    ...
    it("requires mandatory production security keys", () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("NEXT_PUBLIC_ANDROID_PACKAGE_NAME is required"),
      );
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("FIREBASE_APP_ID_ANDROID is required"),
      );
    });
    ...
    it("fails if GOOGLE_SERVICE_ACCOUNT_JSON is missing when ENFORCE_APP_CHECK is true", () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining(
          "GOOGLE_SERVICE_ACCOUNT_JSON is required when ENFORCE_APP_CHECK=true",
        ),
      );
    });
    ...
    it("warns if HOSTNAME=0.0.0.0 in production without proxy indicators", () => {
      ...
      validateEnvironment();
      expect(console.warn).toHaveBeenCalledWith(
        expect.stringContaining("SECURITY: HOSTNAME=0.0.0.0 in production"),
      );
    });
