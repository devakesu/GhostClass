...
import { contactSchema, processContactSubmission } from "../service";
...
describe("processContactSubmission", () => {
  ...
  it("continues if user confirmation email fails (non-fatal)", async () => {
    ...
    expect(logger.warn).toHaveBeenCalledWith(
      expect.stringContaining("Failed to send user confirmation email"),
      expect.any(Error),
    );
  });
  ...
  it("sanitizes message for email", async () => {
    ...
    expect(renderContactAdminEmail).toHaveBeenCalledWith(
      expect.objectContaining({
