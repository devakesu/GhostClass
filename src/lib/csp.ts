...
export const getCspHeader = (nonce?: string) => {
  ...
  const supabaseCfProxyOrigin = parseProxyOrigin(
    process.env.NEXT_PUBLIC_SUPABASE_CF_PROXY_URL,
  );
  const supabaseAwsProxyOrigin = parseProxyOrigin(
    process.env.NEXT_PUBLIC_SUPABASE_AWS_PROXY_URL,
  );
  const supabaseDevProxyOrigin = parseProxyOrigin(
  ...
  if (!isDev && !nonce) {
    ...
    Sentry.captureException(
      new Error(
        "[CSP] FATAL: Nonce missing in production — refusing to serve degraded CSP",
      ),
      {
        level: "fatal",
        tags: {
          type: "csp_missing_nonce",
          location: "getCspHeader",
