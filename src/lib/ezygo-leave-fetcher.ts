...
export async function fetchLeaveData(token: string) {
  ...
  const [
    ...
    fetchEzygoData<unknown>("/studentleaves", token).catch((e) => {
      logger.error("[EzyGo] Failed to fetch studentleaves", {
        error: String(e),
      });
      throw new Error(
    ...
    fetchEzygoData<unknown>("/usersubgroups", token).catch((e) => {
      logger.error("[EzyGo] Failed to fetch usersubgroups", {
        error: String(e),
      });
      throw new Error(
        `Failed to fetch usersubgroups: ${
          e instanceof Error ? e.message : String(e)
        }`,
      );
    }),
    fetchEzygoData<unknown>("/attendancetypes", token).catch((e) => {
      logger.error("[EzyGo] Failed to fetch attendancetypes", {
        error: String(e),
      });
      throw new Error(
        `Failed to fetch attendancetypes: ${
          e instanceof Error ? e.message : String(e)
        }`,
      );
    }),
    ...
    fetchEzygoData<unknown>("/events", token).catch((e) => {
      ...
      throw new Error(
        `Failed to fetch events: ${e instanceof Error ? e.message : String(e)}`,
      );
    }),
    fetchEzygoData<unknown>(
      "/institution/setting/mandatory_event_coordinator",
      token,
    ).catch((e) => {
      logger.error("[EzyGo] Failed to fetch mandatory_event_coordinator", {
        error: String(e),
      });
      throw new Error(
        `Failed to fetch mandatory_event_coordinator: ${
          e instanceof Error ? e.message : String(e)
        }`,
      );
    }),
    fetchEzygoData<unknown>(
      ...
      logger.error("[EzyGo] Failed to fetch student_leave_approval_level", {
        error: String(e),
      });
      throw new Error(
        `Failed to fetch student_leave_approval_level: ${
          e instanceof Error ? e.message : String(e)
        }`,
      );
    }),
