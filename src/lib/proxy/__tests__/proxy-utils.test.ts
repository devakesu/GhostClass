...
describe("Proxy Utils", () => {
  ...
  describe("readWithLimit", () => {
    ...
    it("throws UpstreamResponseTooLargeError if limit exceeded", async () => {
      ...
      await expect(readWithLimit(stream, 15)).rejects.toThrow(UpstreamResponseTooLargeError);
    });
  ...
  describe("resolveSafeUpstreamErrorMessage", () => {
    it("returns fallback for empty body", () => {
      expect(resolveSafeUpstreamErrorMessage("", 500)).toBe("Upstream service error");
    ...
    it("sanitizes whitespace in plain text", () => {
      const body = "Line 1\nLine 2\tEnd";
      expect(resolveSafeUpstreamErrorMessage(body, 400)).toBe("Line 1 Line 2 End");
    });
    ...
    it("handles no environment variables", () => {
      ...
      expect(targets.every(t => t.name !== "primary" && t.name !== "secondary")).toBe(true);
    });
    ...
    it("returns sanitized plain text if JSON has no known fields", () => {
      ...
      expect(resolveSafeUpstreamErrorMessage(body, 400)).toBe('{"other":"data"}');
    });
