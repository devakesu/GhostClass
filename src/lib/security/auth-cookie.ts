...
function parseAndDecryptCachedToken(
  cachedData: { iv: string; content: string } | string,
): string | null {
  ...
  } else if (cachedData?.iv && cachedData?.content) {
    return decrypt(cachedData);
  }
  return null;
}

async function tryGetTokenFromCache(
  finalUserId: string,
): Promise<string | null> {
  const cacheKey = `healed_token:${finalUserId}`;
  try {
    const cachedData = await redis.get<
      { iv: string; content: string } | string
    >(cacheKey);
    if (!cachedData) return null;

    const cachedToken = parseAndDecryptCachedToken(cachedData);
    if (cachedToken) {
...
export async function getAuthTokenWithFallback(userId?: string) {
  ...
  try {
    ...
    // Try Redis cache first to avoid DB query penalty
    const cachedToken = await tryGetTokenFromCache(finalUserId);
    ...
    if (dbUser?.ezygo_token && dbUser?.ezygo_iv) {
      ...
      // Encrypt before caching in Redis for 5 minutes (300 seconds)
      const cacheKey = `healed_token:${finalUserId}`;
      try {
        const encryptedCachePayload = encrypt(token);
        await redis.set(cacheKey, encryptedCachePayload, { ex: 300 });
      } catch (redisErr) {
        logger.dev(
          "[auth-cookie] Failed to cache encrypted token in Redis",
          redisErr,
        );
      }
    ...
    Sentry.captureException(toError(err), {
      tags: { location: "auth-cookie/getAuthTokenWithFallback" },
    });
  }
