...
describe("performProfileSync", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(getAdminClient).mockReturnValue(
      mockSupabase as unknown as ReturnType<typeof getAdminClient>,
    );
    ...
    vi.mocked(egressFetch).mockImplementation(async (url: unknown) => {
      if (url === "myprofile") {
        return createMockResponse(
          '{"data": {"user_id": "12345", "username": "testuser", "full_name": "Test User"}}',
        );
      }
      ...
      if (url === "institutionuser/courses/withusers") {
        return createMockResponse(
          '[{"id": 101, "code": "CS101", "name": "Intro to CS", "usersubgroup": {"id": 201, "name": "Class A", "programme_config_group_id": 710, "usergroup": {"id": 301, "name": "Computer Science"}}}]',
        );
      }
  ...
  it("handles missing EzyGo User ID", async () => {
    ...
    await expect(performProfileSync(mockToken, "", mockAuthId)).rejects.toThrow(
      "Missing EzyGo User ID",
    );
  });
  ...
  it("performs a sync for the Even semester", async () => {
    vi.mocked(egressFetch).mockImplementation(async (url: unknown) => {
      ...
      if (url === "user/setting/default_semester") {
        return createMockResponse("Even");
      }
      return createMockResponse("{}");
    });
    vi.mocked(safeResponseJson).mockResolvedValue({ user_id: "12345" });
  ...
  it("does not create a class from subgroupRoles when courses are empty", async () => {
    vi.mocked(egressFetch).mockImplementation(async (url: unknown) => {
      ...
      if (url === "institutionuser/courses/withusers") {
        return createMockResponse("[]"); // No courses
      }
      if (url === "institutionuser/myroles") {
        return createMockResponse(
          '{"data": {"subgroupRoles": [{"id": 999, "name": "Fallback Class"}]}}',
        );
      }
      return createMockResponse("{}");
    });
    vi.mocked(safeResponseJson).mockImplementation(async (res: unknown) => {
      const text = await (res as Response).text();
      if (text.includes("subgroupRoles")) {
        return {
          data: { subgroupRoles: [{ id: 999, name: "Fallback Class" }] },
        };
      }
  ...
  it("handles safeEzygoJson read failure", async () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith(
      expect.stringContaining("safeEzygoJson: failed to read response body:"),
      expect.any(Error),
    );
    ...
    vi.mocked(egressFetch).mockImplementation(async (url: unknown) => {
      ...
      if (url === "institutionuser/courses/withusers") {
        return {
          ...
          status: 200,
          clone: () => ({
            text: () => Promise.reject(new Error("Clone Read Error")),
          }),
        } as unknown as Response;
      }
      return createMockResponse("{}");
  ...
  it("resolves ezygoId from remote data if local is missing", async () => {
    vi.mocked(egressFetch).mockImplementation(async (url: unknown) => {
      if (url === "myprofile") {
        return createMockResponse('{"user_id": "remote-123"}');
      }
      return createMockResponse("{}");
    });
  ...
  it("resolves ezygoId from user.id if user_id is missing", async () => {
    vi.mocked(egressFetch).mockImplementation(async (url: unknown) => {
      if (url === "myprofile") {
        return createMockResponse('{"user": {"id": "nested-123"}}');
      }
      return createMockResponse("{}");
    });
    vi.mocked(safeResponseJson).mockResolvedValue({
  ...
  it("derives first and last name from full_name", async () => {
    vi.mocked(egressFetch).mockImplementation(async (url: unknown) => {
      if (url === "myprofile") {
        return createMockResponse(
          '{"user_id": "123", "full_name": "John Quincy Adams"}',
        );
      }
      return createMockResponse("{}");
    });
  ...
  it("uses sex and dob as fallbacks for gender and birthDate", async () => {
    vi.mocked(egressFetch).mockImplementation(async (url: unknown) => {
      if (url === "myprofile") {
        return createMockResponse(
          '{"user_id": "123", "sex": "Female", "dob": "1995-05-05"}',
        );
      }
      return createMockResponse("{}");
    });
  ...
  it("skips course mapping if code is missing", async () => {
    vi.mocked(egressFetch).mockImplementation(async (url: unknown) => {
      if (url === "myprofile") return createMockResponse('{"user_id": "123"}');
      if (url === "institutionuser/courses/withusers") {
        return createMockResponse('[{"id": 101, "name": "No Code"}]');
      }
      return createMockResponse("{}");
    });
    vi.mocked(safeResponseJson).mockImplementation(async (res: unknown) => {
  ...
  it("handles class upsert error (priority 1)", async () => {
    vi.mocked(egressFetch).mockImplementation(async (url: unknown) => {
      ...
      if (url === "institutionuser/courses/withusers") {
        return createMockResponse(
          '[{"id": 101, "code": "C1", "usersubgroup": {"id": 1, "name": "G1", "programme_config_group_id": 710, "usergroup": {"id": 1, "name": "Programme A"}}}]',
        );
      }
      return createMockResponse("{}");
    });
  ...
  it("handles class upsert error (priority 2 fallback)", async () => {
    vi.mocked(egressFetch).mockImplementation(async (url: unknown) => {
      ...
      if (url === "institutionuser/myroles") {
        return createMockResponse(
          '{"data": {"subgroupRoles": [{"id": 1, "name": "R1"}]}}',
        );
      }
      return createMockResponse("{}");
    });
    vi.mocked(safeResponseJson).mockImplementation(async (res: unknown) => {
      const text = await (res as Response).text();
      if (text.includes("R1")) {
        return { data: { subgroupRoles: [{ id: 1, name: "R1" }] } };
      }
      return { user_id: "123" };
    });
  ...
  it("handles coursesRes not ok", async () => {
    vi.mocked(egressFetch).mockImplementation(async (url: unknown) => {
      if (url === "myprofile") return createMockResponse('{"user_id": "123"}');
      if (url === "institutionuser/courses/withusers") {
        return createMockResponse("", false, 403);
      }
      return createMockResponse("{}");
    });
    vi.mocked(safeResponseJson).mockResolvedValue({ user_id: "123" });
  ...
  it("skips course mapping if id is missing", async () => {
    vi.mocked(egressFetch).mockImplementation(async (url: unknown) => {
      if (url === "myprofile") return createMockResponse('{"user_id": "123"}');
      if (url === "institutionuser/courses/withusers") {
        return createMockResponse('[{"code": "C1", "name": "No ID"}]');
      }
      return createMockResponse("{}");
    });
    vi.mocked(safeResponseJson).mockImplementation(async (res: unknown) => {
  ...
  it("skips class upsert when no display name is available (priority 1)", async () => {
    vi.mocked(egressFetch).mockImplementation(async (url: unknown) => {
      ...
      if (url === "institutionuser/courses/withusers") {
        ...
        return createMockResponse(
          '[{"id": 101, "code": "CS101", "usersubgroup": {"id": 1, "programme_config_group_id": 710, "usergroup": {"id": 301}}}]',
        );
      }
      if (url === "institutionuser/myroles") {
        return createMockResponse('{"data": {"subgroupRoles": []}}');
    ...
    mockSupabase.single.mockResolvedValue({
      data: { id: "fallback-uuid-710", name: "710" },
      error: null,
    });
  ...
  it("keeps EzyGo default academic context as source of truth even when course cohort differs", async () => {
    ...
    mockSupabase.maybeSingle.mockResolvedValue({ data: null, error: null }); // existingUser
    mockSupabase.single.mockResolvedValue({
      data: { id: "cloned-class-uuid", name: "CS1B2025-2029 Batch even S2" },
      error: null,
    }); // classData
