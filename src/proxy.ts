...
function clearSessionCookies(res: NextResponse, request: NextRequest) {
  ...
  try {
    ...
    logger.warn(
      "Non-critical: Failed to clear some session cookies in middleware",
      e,
    );
  }
}
...
function redirectWithProxyHeaders(
  url: URL,
  status: number,
  cspHeader: string,
  nonce: string,
...
async function getUserWithRetry(
  ...
  try {
    ...
    if (isTransient) {
      logger.warn("Supabase getUser network failure, retrying once...", {
        error,
      });
  ...
  try {
    ...
    if (dbUser?.ezygo_token && dbUser?.ezygo_iv) {
      ...
      if (maxAge > 0) {
        ...
        logger.info("EzyGo session cookie self-healed in middleware", {
          userId: redact("id", userId),
        });
      } else {
        logger.warn(
          "EzyGo session token in DB is expired, skipping self-heal",
          { userId: redact("id", userId) },
        );
      }
    }
...
async function resolveSessionUser(
  ...
  try {
    const { data, error } = await getUserWithRetry(
      supabase as unknown as Parameters<typeof getUserWithRetry>[0],
    );
    if (error) {
      if (
    ...
    logger.warn("Supabase auth getUser threw unexpectedly in middleware", {
      error,
    });
    return { user: null, isUnauthenticatedCertain: true };
  }
...
async function enforceRoutingScenarios({
  ...
  if (isUnauthenticatedCertain && (isProtectedRoute || isAcceptTermsRoute)) {
    ...
    url.pathname = "/";
    const redirectRes = redirectWithProxyHeaders(
      url,
      redirectStatus,
      cspHeader,
      nonce,
  ...
  if (
    ...
    const rawRedirectCount = parseInt(
      request.cookies.get("terms_redirect_count")?.value ?? "0",
      10,
    );
    const redirectCount = Number.isFinite(rawRedirectCount)
      ? rawRedirectCount
      : 0;
    ...
    if (redirectCount >= 3) {
      ...
      homeUrl.pathname = "/";
      homeUrl.search = "";
      const logoutRes = redirectWithProxyHeaders(
        homeUrl,
        redirectStatus,
        cspHeader,
        nonce,
