...
const { generateFirebaseJson, generateFirebaseOptionsDart } = require('../../../scripts/generate-firebase-json.js');
...
describe('generate-firebase-json script', () => {
  ...
  it('generates standard firebase.json and firebase_options.dart from env variables', () => {
    ...
    expect(jsonContent.flutter.platforms.android.default.projectId).toBe('test-project-123');
    expect(jsonContent.flutter.platforms.android.default.appId).toBe('1:12345:android:abcdef');
    expect(jsonContent.flutter.platforms.dart['lib/firebase_options.dart']).toBeDefined();
  });

  it('generates multi-flavor firebase.json from secrets array', () => {
    const secrets = [
      { secretKey: 'FIREBASE_PROJECT_ID', secretValue: 'nexus-project' },
      { secretKey: 'FIREBASE_ANDROID_APP_ID_NEXUS', secretValue: '1:999:android:nexus' },
      { secretKey: 'FIREBASE_IOS_APP_ID_NEXUS', secretValue: '1:999:ios:nexus' },
      { secretKey: 'FIREBASE_ANDROID_APP_ID_NEXUS_MEC', secretValue: '1:999:android:mec' },
      { secretKey: 'FIREBASE_IOS_APP_ID_NEXUS_MEC', secretValue: '1:999:ios:mec' }
    ];
    ...
    const jsonContent = JSON.parse(fs.readFileSync(tempJsonFile, 'utf8'));
    expect(jsonContent.flutter.platforms.android.default.appId).toBe('1:999:android:nexus');
    expect(jsonContent.flutter.platforms.dart['lib/firebase_options_nexus.dart']).toBeDefined();
    expect(jsonContent.flutter.platforms.dart['lib/firebase_options_nexus_mec.dart']).toBeDefined();
  });
  ...
  it('generates valid firebase_options.dart code content', () => {
    ...
    expect(content).toContain("apiKey: String.fromEnvironment('FIREBASE_API_KEY_ANDROID')");
    expect(content).toContain("projectId: String.fromEnvironment('FIREBASE_PROJECT_ID')");
  });
});
